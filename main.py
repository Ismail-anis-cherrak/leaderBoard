...
coefficients = [1,1,1,1,1,1,1,1]  # default coefficients
sum_coefs = sum(coefficients)
...
# def send_scores_to_supabase(scores, table_name):
#     for team_name, score in scores.items():
#         data = {
#             "team_id": team_name.lower().replace(" ", "_"),  # or use actual teamId if preferred
#             "team_name": team_name,
#             "score": int(score * 1e5)  # convert float to bigint
#         }
#         response = supabase.table(table_name).insert(data).execute()
#         if response.error:
#             print(f"Error inserting {team_name} into {table_name}: {response.error}")
#         else:
#             print(f"Inserted {team_name} into {table_name}")

# # Send public and private scores
# send_scores_to_supabase(public_scores, "public_leaderboards")
# send_scores_to_supabase(private_scores, "private_leaderboards")

# print("\n✅ All scores sent to Supabase!")
